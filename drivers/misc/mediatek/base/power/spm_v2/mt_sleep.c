...
/*Dump GPIO to /data/PINdumpxxx.dump*/
#include "6755_gpio.h"
#endif
...
void gpio_dump_regs_func(void)
{
	...
	slp_error("PIN: [MODE] [PULL_SEL] [DIN] [DOUT] [PULL EN] [DIR] [IES] [SMT] [DRV]\n");
	for (idx = MT_GPIO_START; idx < MT_GPIO_MAX; idx++) {
		slp_error("idx = %3d: %d %d %d %d %d %d %d %d %d\n",
				...
				mt_get_gpio_ies(idx), mt_get_gpio_smt(idx),mt_get_gpio_driving(idx));
	}
}
...
static int slp_suspend_ops_enter(suspend_state_t state)
{
	int ret = 0;
#ifdef CONFIG_FIH_DUMP_GPIO2FILE
	extern char *buf_gpio_slp;
	extern int g_sleepdump_value;
	extern unsigned int g_fih_sleepdump_len;
#endif
	...
	if (g_sleepdump_value)
		g_fih_sleepdump_len = mt_gpio_dump_regs(buf_gpio_slp, PAGE_SIZE);
#endif
