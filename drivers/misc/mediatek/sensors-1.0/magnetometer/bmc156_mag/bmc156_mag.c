...
/*----------------------------------------------------------------------------*/
static ssize_t cpsdata_value(int data[3])
{
	struct bmm150api_mdata_s32 mdata;
	s32 mag[3];
	...
	/*remap coordinate*/
	mag[obj->cvt.map[0]] = obj->cvt.sign[0]*mdata.datax;
	mag[obj->cvt.map[1]] = obj->cvt.sign[1]*mdata.datay;
	mag[obj->cvt.map[2]] = obj->cvt.sign[2]*mdata.dataz;
...
/*----------------------------------------------------------------------------*/
static int bmm150_read_chipid(struct i2c_client *client)
{
	/*int err = 0;*/
	u8 chip_id = 0;
...
static long bmm150_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MSENSOR_IOCTL_READ_CHIPINFO:		/* reserved */
		...
		chip_id = bmm150_read_chipid(this_client);
		sprintf(buff, "%d", chip_id);
...
static int bmm150_wakeup(struct i2c_client *client)
{
	...
	if (try_times == 0) {
		client->addr = 0x13;
		try_times = BMM_MAX_RETRY_WAKEUP;
		while (try_times) {
			err = i2c_master_send(client, data, 2);
			mdelay(BMM_I2C_WRITE_DELAY_TIME);
			dummy = 0;
...
static int bmc156_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	int value = 0;
	...
	MSE_LOG("akm8963 mag set delay = (%d) ok.\n", value);
	return 0;
}

static int bmc156_flush(void)
{
...
static int bmm150_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct bmm150_i2c_data *data = NULL;
	int err = 0;
	...
	data = kzalloc(sizeof(struct bmm150_i2c_data), GFP_KERNEL);
	if (!data) {
		err = -ENOMEM;
		goto exit;
	}
...
#ifdef USE_NEW_SENSOR_ARCH
	ctl.enable = bmc156_m_enable;
	ctl.set_delay = bmc156_m_set_delay;
	ctl.open_report_data = bmc156_m_open_report_data;
	//ctl.o_enable = bmc156_o_enable;
	//ctl.o_set_delay = bmc156_o_set_delay;
	//ctl.o_open_report_data = bmc156_o_open_report_data;
	ctl.batch = bmc156_batch;
	ctl.flush = bmc156_flush;
	...
	mag_data.get_data = bmc156_m_get_data;
