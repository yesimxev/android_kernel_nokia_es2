...
static int BMA255_CheckDeviceID(struct i2c_client *client)
{
	...
	if (databuf[0] != BMA255_FIXED_DEVID) {
		GSE_ERR("BMA255_CheckDeviceID %d, %d failt!\n ", databuf[0], databuf[1]);
...
static int bmc156_acc_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
		GSE_LOG("bma2x2 acc set delay = (%lld) ok.\n", samplingPeriodNs);
		return 0;
}

static int bmc156_acc_flush(void)
{
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ctl.batch = bmc156_acc_batch;
	ctl.flush = bmc156_acc_flush;
