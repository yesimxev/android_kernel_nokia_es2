...
struct bmi160_acc_i2c_data {
	...
	struct work_struct init_fifo_work;
 ...
 * @return zero success, non-zero failed
 */
static void bmi160_acc_init_client(struct work_struct *work)
{
	int err = 0;
	struct bmi160_acc_i2c_data *obj = obj_i2c_data;
	...
	if (err < 0)
		goto EXIT_ERR;
	bmi_delay(5);
	err = bmi160_acc_set_odr(obj->client, BMI160_ACCEL_ODR_200HZ);
	if (err < 0)
		goto EXIT_ERR;
	err = bmi160_acc_set_osr4(obj->client);
	if (err < 0)
		goto EXIT_ERR;
	err = bmi160_acc_set_data_range(obj->client, BMI160_ACCEL_RANGE_4G);
	if (err < 0)
		goto EXIT_ERR;
	gsensor_gain.x = gsensor_gain.y = gsensor_gain.z =
	obj->reso->sensitivity;
	err = bmi160_acc_set_int_enable(obj->client, 0);
	if (err < 0)
		goto EXIT_ERR;
	...
	err = bmi160_acc_reset_calib(obj->client);
	if (err < 0)
		goto EXIT_ERR;
	...
	struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (err) {
		...
		acc[BMI160_ACC_AXIS_X] -= cali_offset[BMI160_ACC_AXIS_X];
		acc[BMI160_ACC_AXIS_Y] -= cali_offset[BMI160_ACC_AXIS_Y];
		acc[BMI160_ACC_AXIS_Z] -= cali_offset[BMI160_ACC_AXIS_Z];
...
static int bmi160_acc_read_sensor_data_for_cali(
	struct i2c_client *client, char *buf, int bufsize)
{
	int err = 0;
	int acc[BMI160_ACC_AXES_NUM] = { 0 };
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE, "%d %d %d\n",
		 cali_offset[BMI160_ACC_AXIS_X],
		 cali_offset[BMI160_ACC_AXIS_Y],
		 cali_offset[BMI160_ACC_AXIS_Z]);
...
{
	int x, y, z;
	int sum_data[BMI160_ACC_AXES_NUM] = {0,0,0}, avg_data[BMI160_ACC_AXES_NUM];
	...
	while(loop_times) {
		...
		err = bmi160_acc_read_sensor_data_for_cali(
...
static void bmi160_init(struct work_struct *work)
{
	/* variable used for return the status of communication result*/
	//struct bmi160_acc_i2c_data *p_bmi160_data = obj_i2c_data;
	BMI160_RETURN_FUNCTION_TYPE com_rslt = E_BMI160_COMM_RES;
	u8 v_data_u8 = BMI160_INIT_VALUE;
	u8 v_pmu_data_u8 = BMI160_INIT_VALUE;
	int count = 0;
	/* assign bmi160 ptr */
	p_bmi160 = &obj_i2c_data->device;
	com_rslt =
	p_bmi160->BMI160_BUS_READ_FUNC(
...
static void bmi160_acc_init_fifo(struct work_struct *work)
{
	...
	const char *buf, size_t count)
{
	...
	u8 acc_odr, range = 0, acc_selftest_amp, acc_selftest_sign;
	struct bmi160_acc_i2c_data *client_data = obj_i2c_data;
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_SET_CALI:
		...
		if (copy_from_user(
			&cali_data, data, sizeof(cali_data))) {
			err = -EFAULT;
			break;
		}
		if (atomic_read(&obj->suspend)) {
			...
			cali[BMI160_ACC_AXIS_X] = cali_data[0];
			cali[BMI160_ACC_AXIS_Y] = cali_data[1];
			cali[BMI160_ACC_AXIS_Z] = cali_data[2];
		}
		break;
	...
	GSE_ERR("[FIH]cmd = GSENSOR_IOCTL_GET_CALI\n");
		data = (void __user *)arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
...
static int bmi160_factory_set_cali(int32_t cali_data[3])
{
/*
	int err = 0;
	int cali[3] = { 0 };
...
*/
	GSE_ERR("%s bmi160_factory_set_cali\n", __func__);

	cali_offset[BMI160_ACC_AXIS_X] = cali_data[0]/1000;
	cali_offset[BMI160_ACC_AXIS_Y] = cali_data[1]/1000;
	cali_offset[BMI160_ACC_AXIS_Z] = cali_data[2]/1000;
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_WORK(&obj->init_work, bmi160_init);
	schedule_work(&obj->init_work);

	atomic_set(&obj->trace, 0);
	err = bmi160_acc_check_chip_id(new_client);
	if (err < 0)
		goto exit_init_failed;
	INIT_WORK(&obj->init_client_work, bmi160_acc_init_client);
	schedule_work(&obj->init_client_work);
	...
	/* fifo setting*/
	INIT_WORK(&obj->init_fifo_work, bmi160_acc_init_fifo);
	schedule_work(&obj->init_fifo_work);
