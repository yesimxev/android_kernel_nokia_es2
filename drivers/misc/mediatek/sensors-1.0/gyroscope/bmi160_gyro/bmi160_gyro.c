...
static void bmg_init_client(struct work_struct *work)
{
	struct bmg_i2c_data *obj =
		(struct bmg_i2c_data *)obj_i2c_data;
	int err = 0;

	err = bmg_set_datarate(obj->client, BMI160_GYRO_ODR_100HZ);
	if (err < 0)
		goto EXIT_ERR;
	err = bmg_set_range(obj->client, (enum BMG_RANGE_ENUM)BMG_RANGE_2000);
	if (err < 0)
		goto EXIT_ERR;
	...
	return;
EXIT_ERR:
	GYRO_ERR("bmi160 gyro init failed: %d", err);
	return;
}
		...
		const struct i2c_device_id *id)
{
	...
	if (err < 0)
		pr_err("Gyro device tree configuration error.\n");
...
#endif
	err = bmg_get_chip_type(obj->client);
	if (err < 0)
		goto exit_hwmsen_get_convert_failed;
	INIT_WORK(&obj->init_client_work, bmg_init_client);
	schedule_work(&obj->init_client_work);
