...
/* log macro */
#define BMG160_TAG                  "[gyro_bmg160] "
//#define GYRO_FUN(f)               printk(KERN_INFO BMG_TAG"%s\n", __func__)
#define GYRO_ERR(fmt, args...) \
	printk(KERN_ERR BMG160_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
//#define GYRO_LOG(fmt, args...)    printk(KERN_INFO BMG160_TAG fmt, ##args)

#define DMA_BUFFER_SIZE 1024
//static unsigned char *I2CDMABuf_va = NULL;
//static u8 I2CDMABuf_pa[DMA_BUFFER_SIZE] = {0};
...
#if 0
static ssize_t show_fifo_data_frame_value(struct device *dev, char *buf)
{
	struct bmg_i2c_data *obj = obj_i2c_data;
        int read_cnt = 0;
	...
	if (fifo_framecount == 0)
	{
        ...
        while (read_cnt > 0)
        {
                ...
                pBuf += f_len;
                read_cnt--;
                GYRO_ERR("fifo_frame_cnt %d, f_len %d", fifo_framecount, f_len);
...
static int bmg160_gyro_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	int value = 0;
	...
	return bmg160_gyro_set_delay(samplingPeriodNs);
}

static int bmg160_gyro_flush(void)
{
		...
		const struct i2c_device_id *id)
{
	...
	ctl.batch = bmg160_gyro_batch;
	ctl.flush = bmg160_gyro_flush;
	ctl.is_report_input_direct = false;
	ctl.is_support_batch = obj->hw.is_batch_supported;
	err = gyro_register_control_path(&ctl);
	if(err) {
