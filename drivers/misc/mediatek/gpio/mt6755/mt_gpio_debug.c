...
#ifdef CONFIG_FIH_DUMP_GPIO2FILE
#include <asm/uaccess.h>

#define FIH_GPIO_DUMP_FILE_PATH				"/data/"
#define FIH_GPIO_DUMP_ERROR(fmt, args...)	do { printk(KERN_ERR "[GPIO][DUMP][Error]%s. line: %d.  "fmt"\n", \
...
ssize_t mt_gpio_dump_regs(char *buf, ssize_t bufLen)
{
	int idx = 0, len = 0;
...
#else
	/*char tmp[]="PIN: [MODE] [PULL_SEL] [DIN] [DOUT] [PULL EN] [DIR] [INV] [IES]\n"; */
	char tmp[] = "PIN: [MODE] [PULL_SEL] [DIN] [DOUT] [PULL EN] [DIR] [IES] [SMT] [DRV]\n";
...
#ifdef CONFIG_FIH_DUMP_GPIO2FILE
/*****************************************************************************
 * Add GPIO dump dev_attr APIs by lijl
 ******************************************************************************/
int g_sleepdump_value = 0;
int g_activedump_value = 0;
unsigned int g_fih_sleepdump_len = 0;
unsigned int g_fih_activedump_len = 0;

char *buf_gpio_slp = NULL;
char *buf_gpio_active = NULL;
ssize_t FIH_gpio_show_sleepdump(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_sleepdump_value);
}

ssize_t FIH_gpio_store_sleepdump(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	...
	if (buf != NULL && count != 0) {
		...
		g_sleepdump_value = temp;
	}

	g_fih_sleepdump_len = 0;

	if (g_sleepdump_value) {
		buf_gpio_slp = (char *)kmalloc(PAGE_SIZE, GFP_ATOMIC);

		if (!buf_gpio_slp) {
			printk("kmalloc buf_gpio_slp fail!!!\n");
			g_sleepdump_value = 0;
			return -ENOMEM;
		}
	}

	if ((g_sleepdump_value == 0) && (buf_gpio_slp != NULL)) {
		kfree(buf_gpio_slp);
		buf_gpio_slp = NULL;
	}
...
ssize_t FIH_gpio_show_activedump(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_activedump_value);
}

ssize_t FIH_gpio_store_activedump(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	int temp = 0;
	int ret;
	if (buf != NULL && count != 0) {
		ret = kstrtoint(buf, 10, &temp);
		g_activedump_value = temp;
	}

	g_fih_activedump_len = 0;

	if (g_activedump_value) {
		buf_gpio_active = (char *)kmalloc(PAGE_SIZE, GFP_ATOMIC);

		if (!buf_gpio_active) {
			g_activedump_value = 0;
			printk("kmalloc buf_gpio_active fail!!!\n");
			return -ENOMEM;
		}

	}
	if ((g_activedump_value == 0) && (buf_gpio_active != NULL)) {
		kfree(buf_gpio_active);
		buf_gpio_active = NULL;
	}

	return count;
}

int FIH_gpio_dump_regs2file(char *buf, char *file_name)
{
	...
	int tmp_data_write = 0;

	printk("[GPIO][DUMP]: Enter\n");
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", FIH_GPIO_DUMP_FILE_PATH, file_name);
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	...
	if (IS_ERR(pfile)) {
		FIH_GPIO_DUMP_ERROR("error occured while opening file %s.",  filepath);
		set_fs(old_fs);
		return -EIO;
	}
	...
	if (g_activedump_value) {
		g_fih_activedump_len = mt_gpio_dump_regs(buf, PAGE_SIZE);
		tmp_len = g_fih_activedump_len;
	}

	if (g_sleepdump_value)
		tmp_len = g_fih_sleepdump_len;

	tmp_data_write = vfs_write(pfile, buf, tmp_len, &pos);
	if (!tmp_data_write) {
		pr_err("[gpio_dump_regs2file] write fail\n");
		ret = -1;
	}
	set_fs(old_fs);
...
#ifdef CONFIG_FIH_CHANGE_SMT
		else if (!strncmp(buf, "smt", 3) && (2 == sscanf(buf + 3, "%d %d", &pin, &smt)))
