...
// { 4208, 3120,	184,  120, 3840, 2880,  640,  480,   0,	0,  640,  480, 	 0, 0,  640,  480}, //hight speed video
 /* set high speed mode sensor output to 1280x720 to solve picture poor quality issue*/
 { 4208, 3120,	184,  480, 3840, 2160, 1280,  720,   0,	0, 1280,  720, 	 0, 0, 1280,  720},// high speed video
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("S5K3L8mipiraw_sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
