...
struct rt5081a_pmu_charger_data *g_chg_data;
...
int rt5081a_ship_mode_enable(bool en)
{
	int ret = 0;

	dev_info(g_chg_data->dev, "%s: en = %d\n", __func__, en);
	ret = (en ? rt5081a_pmu_reg_set_bit : rt5081a_pmu_reg_clr_bit)
		(g_chg_data->chip, RT5081A_PMU_REG_CHGCTRL2, 1 << 7);

	return ret;
}
EXPORT_SYMBOL(rt5081a_ship_mode_enable);
...
#if defined(CONFIG_DEBUG_REG)
static int rt5081a_addr_value;
static struct rt5081a_pmu_chip *rt5081a_chip;

static ssize_t rt5081a_addr_read(struct device *dev,
			     struct device_attribute *attr,
			     char *buf)
{
	pr_err("in %s----rt5081a_addr_value=%x---------wxs--\n", __func__, rt5081a_addr_value);

	return scnprintf(buf, PAGE_SIZE, "%d\n", rt5081a_addr_value);
}

static ssize_t rt5081a_addr_write(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{

	rt5081a_addr_value = simple_strtoul(buf, NULL, 10);
	pr_err("in %s----rt5081a_addr_value=%x---------wxs--\n", __func__, rt5081a_addr_value);

	return count;
}

static ssize_t rt5081a_value_read(struct device *dev,
			     struct device_attribute *attr,
			     char *buf)
{
	int ret;

	ret = rt5081a_pmu_reg_read(rt5081a_chip, rt5081a_addr_value);

	pr_err("in %s----ret=%x---------wxs--\n", __func__, ret);

	return scnprintf(buf, PAGE_SIZE, "%d\n", ret);
}

static ssize_t rt5081a_value_write(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int value, ret;
	value = simple_strtoul(buf, NULL, 10);
	pr_err("in %s----value=%x---------wxs--\n", __func__, value);

	ret = rt5081a_pmu_reg_write(rt5081a_chip, rt5081a_addr_value,
		value);
...
static ssize_t rt5081a_write_addr_write(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int addr, value, ret = 0;
	if (!strncmp(buf, "-w", 2)) {
			buf += 2;
			if (2 == sscanf(buf, "%d %d", &addr, &value))
				ret = rt5081a_pmu_reg_write(rt5081a_chip, addr,
					value);
		if (ret < 0)
			pr_err("%s:  failed\n", __func__);
		}
...
static struct device_attribute rt5081a_sysfs_attrs[] = {
	__ATTR(rt5081a_addr, (S_IRUGO | S_IWUSR | S_IWGRP),
		rt5081a_addr_read, rt5081a_addr_write),
	__ATTR(rt5081a_value, (S_IRUGO | S_IWUSR | S_IWGRP),
		rt5081a_value_read, rt5081a_value_write),
	__ATTR(rt5081a_write_addr, (S_IRUGO | S_IWUSR | S_IWGRP),
		NULL, rt5081a_write_addr_write),
};
#endif
static int rt5081a_pmu_charger_probe(struct platform_device *pdev)
{
	...
	g_chg_data = chg_data;
	#if defined(CONFIG_DEBUG_REG)
	__rt5081a_enable_safety_timer(chg_data,0);

	rt5081a_chip = chg_data->chip;

	for (i = 0; i < ARRAY_SIZE(rt5081a_sysfs_attrs); i++)
		if (device_create_file(&pdev->dev,
					   &rt5081a_sysfs_attrs[i]))
			goto sysfs_psy_create_attrs_failed_rt5081a;
	#endif
...
#if defined(CONFIG_DEBUG_REG)
sysfs_psy_create_attrs_failed_rt5081a:
	pr_err("Failed creating sysfs psy attrs for rt5081a.\n");
	while (i--)
		device_remove_file(&pdev->dev, &rt5081a_sysfs_attrs[i]);
#endif
	return ret;
}
...
static int rt5081a_pmu_charger_remove(struct platform_device *pdev)
{
	...
	#if defined(CONFIG_DEBUG_REG)
	for (i = 0; i < ARRAY_SIZE(rt5081a_sysfs_attrs); i++)
		(void)device_remove_file(&pdev->dev, &rt5081a_sysfs_attrs[i]);
	#endif
	return 0;
}
