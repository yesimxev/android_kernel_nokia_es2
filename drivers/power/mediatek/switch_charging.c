...
unsigned int g_aicr_upper_bound_min;
...
static int mtk_check_aicr_upper_bound(void)
{
	...
	aicr_upper_bound = g_aicr_upper_bound_min * 100;
...
void select_charging_current(void)
{
	...
	if(g_temp_CC_value >= PSE[BMT_status.charge_mode][CHARGE_CUR])
		g_temp_CC_value = PSE[BMT_status.charge_mode][CHARGE_CUR];
	battery_log(BAT_LOG_CRTI, "[BATTERY] Select charging: %d\r\n",g_temp_CC_value);
}
...
void mt_battery_charging_algorithm(void)
{
	...
	if(g_aicr_upper_bound != 0)
		g_aicr_upper_bound_min = g_aicr_upper_bound;
