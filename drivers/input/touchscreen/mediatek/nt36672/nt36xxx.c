...
bool nova_usb_plugin_enable = false;
extern int32_t tpd_usb_plugin(int32_t plugin);

struct kobject *nova_tp_kobj;
#if WAKEUP_GESTURE
uint8_t g_gestureSwitch;
#endif
...
static int8_t nvt_ts_check_chip_ver_trim(void)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		nvt_sw_reset_idle();	//DE suggests : do n times
...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	//---set xdata index to EVENT BUF ADDR---
	buffer[0] = 0xFF;
	buffer[1] = (ts->mmap->EVENT_BUF_ADDR >> 16) & 0xFF;
	buffer[2] = (ts->mmap->EVENT_BUF_ADDR >> 8) & 0xFF;
	CTP_I2C_WRITE(ts->client, I2C_FW_Address, buffer, 3);

	//---read fw info---
	buffer[0] = EVENT_MAP_FWINFO;
	CTP_I2C_READ(ts->client, I2C_FW_Address, buffer, 17);

	num_read_chars = snprintf(buf, PAGE_SIZE, "02_00_%02X_%02X\n", buffer[1], vendor_id);
...
{
    if (buf[0] == '1') {
        NVT_ERR("[GESTURE]enable gesture");
        g_gestureSwitch = 1;
    } else {
        NVT_ERR("[GESTURE]disable gesture");
        g_gestureSwitch = 0;
    }
...
static DEVICE_ATTR(tp_fw_version, S_IRUGO | S_IWUSR, fts_tpfwver_show, NULL);
#if WAKEUP_GESTURE
...
int nova_create_sysfs(struct i2c_client *client)
{
    int ret = 0;

    ret = sysfs_create_group(nova_tp_kobj, &fts_attribute_group);
    if (ret) {
        NVT_ERR("[EX]: sysfs_create_group() failed!!");
        sysfs_remove_group(nova_tp_kobj, &fts_attribute_group);
        return -ENOMEM;
    }
...
int nova_remove_sysfs(struct i2c_client *client)
{
    sysfs_remove_group(nova_tp_kobj, &fts_attribute_group);
    return 0;
}
...
/*
when tp is suspended,tp's power is cut. It does not record
the last power status, so if resumed tp, driver must send
cmd again.
*/
void nova_charger_config(void)
{
	if (!charger_running) {
		NVT_ERR("Charger checker suspended!");
		return;
	}

	NVT_ERR("firefly charger usb is enable = %d (0 disable 1 enable)\n", nova_usb_plugin_enable);
	if (nova_usb_plugin_enable) {
		NVT_ERR("Charger Plugin.");
		tpd_usb_plugin(2);
	} else {
		NVT_ERR("Charger Plugout.");
		tpd_usb_plugin(0);
	}
}

void nova_charger_switch(s32 on)
{
	...
	if (SWITCH_ON == on) {
		if (!charger_running) {
			...
			NVT_ERR("Charger checker started!");
			nova_charger_config();
		} else {
			spin_unlock(&charger_lock);
		}
		...
		if (charger_running) {
			charger_running = 0;
			spin_unlock(&charger_lock);
			NVT_ERR("Charger checker stoped!");
		} else {
			spin_unlock(&charger_lock);
		}
	}
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	nova_charger_switch(SWITCH_ON);

	nova_tp_kobj = kobject_create_and_add("android_tp", NULL);
	if(nova_tp_kobj == NULL) {
		NVT_ERR("%s:focal create kobject is failed\n",__func__);
	}

    ret = nova_create_sysfs(client);
    if (ret) {
...
static int32_t nvt_ts_remove(struct i2c_client *client)
{
	...
	nova_remove_sysfs(client);
	kobject_del(nova_tp_kobj);
	i2c_set_clientdata(client, NULL);
	kfree(ts);
...
static const struct of_device_id nvt_match_table[] = {
	{.compatible = "mediatek,cap_touch_nt36672"},
	{},
};
#endif
