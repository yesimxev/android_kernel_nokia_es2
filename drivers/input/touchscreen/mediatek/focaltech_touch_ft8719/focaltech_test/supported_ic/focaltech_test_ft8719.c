...
enum test_item_ft8719 {
	CODE_FT8719_ENTER_FACTORY_MODE = 0,//All IC are required to test items
...
static int ft8719_short_test(bool *test_result)
{
	...
	save_testdata_incell( adcdata, 0, test_data.screen_param.tx_num + 1, test_data.screen_param.rx_num, 1 );

TEST_END:
	if (temp_result) {
		FTS_TEST_SAVE_INFO_RESULT("\n==========Short Circuit Test is OK!\r\n");
		*test_result = true;
	} else {
...
static int ft8719_open_test(bool *test_result)
{
	...
	if (ft8719_basic_thr.open_test_k1_check) {
		ret = read_reg(FACTORY_REG_K1, &reg_k1);
		if (ret) {
			FTS_TEST_SAVE_INFO("\r\n//=========  Read Reg Failed!");
			goto TEST_ERR_FACTORY;
		}
		ret = write_reg(FACTORY_REG_K1, ft8719_basic_thr.open_test_k1_threshold);
		if (ret) {
			FTS_TEST_SAVE_INFO("\r\n//=========  Write Reg Failed!");
			goto TEST_ERR_FACTORY;
		}
	...
	if (ft8719_basic_thr.open_test_k2_check) {
		ret = read_reg(FACTORY_REG_K2, &reg_k2);
		if (ret) {
			FTS_TEST_SAVE_INFO("\r\n//=========  Read Reg Failed!");
			goto TEST_ERR_FACTORY;
		}
		ret = write_reg(FACTORY_REG_K2, ft8719_basic_thr.open_test_k2_threshold);
		if (ret) {
			FTS_TEST_SAVE_INFO("\r\n//=========  Write Reg Failed!");
			goto TEST_ERR_FACTORY;
		}
	...
	if (ret) {
		FTS_TEST_SAVE_INFO("\r\n//=========  write reg 0x20 failed !");
		goto TEST_ERR_FACTORY;
	}
	ret = write_reg(FACTORY_REG_VGL02, 0x02);
	if (ret) {
		FTS_TEST_SAVE_INFO("\r\n//=========  write reg 0x21 failed !");
		goto TEST_ERR_FACTORY;
	}
	...
	save_testdata_incell(opendata, 0, tx_num, rx_num, 1);

TEST_ERR:
	if (ft8719_basic_thr.open_test_k1_check) {
		ret = write_reg(FACTORY_REG_K1, reg_k1);
		if (ret) {
			FTS_TEST_SAVE_INFO("\r\n//=========  restore K1 fail");
			goto TEST_ERR_FACTORY;
		}
	...
	if (ft8719_basic_thr.open_test_k2_check) {
		ret = write_reg(FACTORY_REG_K2, reg_k2);
		if (ret) {
			FTS_TEST_SAVE_INFO("\r\n//=========  restore K2 fail");
			goto TEST_ERR_FACTORY;
		}
	}
	/* restore 0x20 0x21 reg */
	ret = write_reg(FACTORY_REG_VGH02, reg_vgh);
	if (ret) {
		tmp_result = false;
		FTS_TEST_SAVE_INFO("\r\nrestore 0x20 reg Failed");
	}
	sys_delay(50);
	ret = write_reg(FACTORY_REG_VGL02, reg_vgl);
	if (ret) {
		tmp_result = false;
		FTS_TEST_SAVE_INFO("\r\nrestore 0x21 reg Failed");
	}
	sys_delay(50);
	/* auto clb */
	ret = chip_clb_incell();
...
static int ft8719_cb_test(bool *test_result)
{
	...
	save_testdata_incell(cbdata, 0, tx_num + 1, rx_num, 1);
...
static int ft8719_lcdnoise_test(bool *test_result)
{
	...
	save_testdata_incell(lcdnoise, 0, tx_num + 1, rx_num, 1);
...
static int ft8719_rawdata_test(bool *test_result)
{
	...
	/* Save Test Data to testdata.csv */
	save_testdata_incell(rawdata, 0, tx_num + 1, rx_num, 1);
...
bool start_test_ft8719(void)
{
	...
	for (item_count = 0; item_count < test_data.test_num; item_count++) {
		...
		/* FT8719_SHORT_TEST */
		if (CODE_FT8719_SHORT_CIRCUIT_TEST == test_data.test_item[item_count].itemcode) {
		...
		/* FT8719_Open_TEST */
		if (CODE_FT8719_OPEN_TEST == test_data.test_item[item_count].itemcode) {
		...
		/* FT8719_CB_TEST */
		if (CODE_FT8719_CB_TEST == test_data.test_item[item_count].itemcode) {
		...
		if (CODE_FT8719_LCD_NOISE_TEST == test_data.test_item[item_count].itemcode) {
		...
		/* FT8719_RAWDATA_TEST */
		if (CODE_FT8719_RAWDATA_TEST == test_data.test_item[item_count].itemcode) {
...
void init_basicthreshold_ft8719(char *strIniFile)
{
	char str[512] = {0};
	...
	ft8719_basic_thr.open_test_k1_threshold = fts_atoi(str);
	...
	ft8719_basic_thr.open_test_k2_threshold = fts_atoi(str);

	FTS_TEST_FUNC_EXIT();
}

void init_testitem_ft8719(char  *strIniFile)
{
	char str[512] = {0};
...
void init_detailthreshold_ft8719(char *ini)
{
	FTS_TEST_FUNC_ENTER();
...
void set_testitem_sequence_ft8719(void)
{
	FTS_TEST_FUNC_ENTER();
