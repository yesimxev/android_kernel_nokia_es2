 ...
 *****************************************************************************/
#if FTS_8719_UPGRADE_EN
#define AL2_FCS_COEF                ((1 << 15) + (1 << 10) + (1 << 3))

u16 ftouch_spi_calc_ecc(u8 *pbuf, u16 length)
{
	u16 ecc = 0;
	u16 i, j;
 ...
 ***********************************************************************/
#if FTS_8719_UPGRADE_EN
static int fts_pram_ecc_cal(struct i2c_client *client, u32 len)
{
	int ret = 0;
	u8 reg_val = 0;
	u8 val[2] = {0};
	...
	packet_buf[0] = FTS_ROMBOOT_CMD_ECC;

	packet_buf[1] = BYTE_OFF_16(offset);
	packet_buf[2] = BYTE_OFF_8(offset);
	packet_buf[3] = BYTE_OFF_0(offset);

	packet_buf[4] = BYTE_OFF_16(packet_len);
	packet_buf[5] = BYTE_OFF_8(packet_len);
	packet_buf[6] = BYTE_OFF_0(packet_len);

	ret = fts_i2c_write(client, packet_buf, 7);
	if (ret < 0) {
	...
	msleep(200);
	packet_buf[0] = FTS_CMD_READ_ECC;
	ret = fts_i2c_read(client, packet_buf, 1, val, 2);
	if (ret < 0) {
		FTS_ERROR("read pramboot ecc fail");
		return ret;
	}
	reg_val = ((u16)(val[0] << 8) + val[1]) & 0x0000FFFF;
	return reg_val;
}
#else
...
#if FTS_8719_UPGRADE_EN
	ecc_in_host = ftouch_spi_calc_ecc(buf, len);
#endif
...
static int fts_pram_write_remap(struct i2c_client *client)
{
	...
	/*read out checksum*/
#if FTS_8719_UPGRADE_EN
	ecc_in_tp = fts_pram_ecc_cal(client, pb_len);
#else
