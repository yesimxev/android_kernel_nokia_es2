...
extern char g_guest_str0[128]; 
extern char g_guest_str1[128];
extern char g_guest_str2[128];
extern char g_guest_str3[128];
extern uint8_t id_buf[12];
		...
		size_t len, loff_t *pos)
{
	...
	if(!HX_PROC_SEND_FLAG)
	{
		...
		ret += sprintf(temp_buf + ret, "g_guest_str0: %s\n", g_guest_str0);
		ret += sprintf(temp_buf + ret, "g_guest_str1: %s\n", g_guest_str1);
		ret += sprintf(temp_buf + ret, "g_guest_str2: %s\n", g_guest_str2);
		ret += sprintf(temp_buf + ret, "g_guest_str3: %s\n", g_guest_str3);
		if(copy_to_user(buf, temp_buf, len))
			I("%s,here:%d\n",__func__,__LINE__);
		kfree(temp_buf);
		HX_PROC_SEND_FLAG=1;
