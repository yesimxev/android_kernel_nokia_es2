...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("[ESD]: NoACK=%d, Error Data=%d, Hardware Reset=%d", fts_esdcheck_data.i2c_nack_cnt, fts_esdcheck_data.i2c_dataerror_cnt, fts_esdcheck_data.hardware_reset_cnt);
    return 0;
}
