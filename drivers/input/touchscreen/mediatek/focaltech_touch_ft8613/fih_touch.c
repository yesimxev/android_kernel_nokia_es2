...
extern int32_t touch_selftest(const char *buf, size_t count);
extern int touch_tpfwver_read(char *);
	...
	size_t count, loff_t *ppos)
{
	...
	if(touch_cb.touch_selftest != NULL)
	{
		...
		touch_cb.touch_selftest(buf,count);
	}
	kfree(buf);
	return count;
}
...
static int fih_touch_double_tap_read_show(struct seq_file *m, void *v)
{
	...
	if (touch_cb.touch_double_tap_read != NULL)
	{
		...
		touch_cb.touch_double_tap_read(buf);
		seq_printf(m, "%s", buf);
	}
	return 0;
}
