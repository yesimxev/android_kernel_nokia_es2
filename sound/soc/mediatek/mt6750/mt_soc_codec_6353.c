...
#ifdef CONFIG_FIH_HAC
static void Hac_Amp_Change(bool enable);
#endif
...
#ifdef CONFIG_FIH_HAC
//add hac
struct pinctrl *pinctrl61;
struct pinctrl_state *hac_high;
struct pinctrl_state *hac_low;
#endif
...
static void Hac_Amp_Change(bool enable)
{
	pr_warn("%s %d\n", __func__, enable);
	if (enable) {
		pinctrl_select_state(pinctrl61, hac_high);
	} else {
		pinctrl_select_state(pinctrl61, hac_low);
	}
}


static int Hac_Amp_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_warn("%s\n", __func__);
	return 0;
}

static int Hac_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	mutex_lock(&Ana_Ctrl_Mutex);
	pr_warn("%s\n", __func__);
	if ((ucontrol->value.integer.value[0] == true)){
...
/*1.2w*/
#ifdef CONFIG_AUDIO_AW87318
static void AW87318_MODE2(void)
{
	do {
		pinctrl_select_state(pinctrl24, extPAen_High);
		udelay(2);
		pinctrl_select_state(pinctrl24, extPAen_Low);
		udelay(2);
		pinctrl_select_state(pinctrl24, extPAen_High);
	} while (0);
}
...
#ifdef CONFIG_FIH_HAC
    SOC_ENUM_EXT("Hac_Amp_Switch", Audio_DL_Enum[13], Hac_Amp_Get,
		     Hac_Amp_Set),
#endif
...
static int mtk_mt6331_codec_dev_probe(struct platform_device *pdev)
{
	...
	/* gpio setting */
	pinctrl24 = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(pinctrl24)) {
		ret = PTR_ERR(pinctrl24);
		pr_warn("Cannot find extPA_en pinctrl!\n");
	}

	extPAen_default = pinctrl_lookup_state(pinctrl24, "extPAen_default");
	if (IS_ERR(extPAen_default)) {
		ret = PTR_ERR(extPAen_default);
		pr_warn("Cannot find pinctrl24 extPAen_default!\n");
	}

	extPAen_High = pinctrl_lookup_state(pinctrl24, "extPAenHigh");
	if (IS_ERR(extPAen_High)) {
		ret = PTR_ERR(extPAen_High);
		pr_warn("Cannot find pinctrl24 extPAenHigh!\n");
	}

	extPAen_Low = pinctrl_lookup_state(pinctrl24, "extPAenLow");
	if (IS_ERR(extPAen_Low)) {
		ret = PTR_ERR(extPAen_Low);
		pr_warn("Cannot find pinctrl24 extPAenLow!\n");
	}

	pinctrl_select_state(pinctrl24, extPAen_Low);
	pr_warn("Ext_Speaker_Amp should be closed in initial state.\n");
#ifdef CONFIG_FIH_HAC

	pinctrl61 = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(pinctrl61)) {
		ret = PTR_ERR(pinctrl61);
		pr_warn("Cannot find hac_en pinctrl!\n");
	}

	hac_high = pinctrl_lookup_state(pinctrl61, "hac_high");
	if (IS_ERR(hac_high)) {
		ret = PTR_ERR(hac_high);
		pr_warn("Cannot find pinctrl61 hac_high!\n");
	}

	hac_low = pinctrl_lookup_state(pinctrl61, "hac_low");
	if (IS_ERR(hac_low)) {
		ret = PTR_ERR(hac_low);
		pr_warn("Cannot find pinctrl61 hac_low!\n");
	}

	pinctrl_select_state(pinctrl61, hac_low);
	pr_warn("hac should be closed in initial state.\n");
#endif
